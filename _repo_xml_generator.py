...
# Comment out this line if you have .pyc or .pyo files you need to keep
    ...
    def _create_zips(self, addon_id, version):
        ...
        if not os.path.exists(final_zip):
            ...
            zp = zipfile.ZipFile(final_zip, 'w', compression=zipfile.ZIP_DEFLATED)
            ...
            for root, dirs, files in os.walk(addon_id):
                ...
                for f in files:
                    ...
                    zp.write(fullpath, archive_name, zipfile.ZIP_DEFLATED)
...
# Remove any instances of pyc or pyo files
    ...
    def _generate_addons_file(self):
        ...
        for addon in addons:
            try:
                if (not os.path.isdir(addon) or addon == "repo" or addon == "zips"
                        or addon.startswith('.')):
                    continue
        ...
        self._save_file(addons_xml.encode('utf-8'),
                        file=os.path.join('zips', 'addons.xml'), decode=True)

    def _generate_md5_file(self):
        try:
            m = hashlib.md5(open(os.path.join('zips', 'addons.xml'), 'r',
